...
if(ROS_IS_ROS2)
  ...
  set(GAZEBO_MIRROR
      "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/pkgs-osrf-archive-keyring.gpg] http://packages.osrfoundation.org/gazebo/ubuntu-stable $(lsb_release -cs)"
  )
  execute_process(
    COMMAND
      sudo bash -c
      "echo \"${GAZEBO_MIRROR} main\" | sudo tee /etc/apt/sources.list.d/gazebo-stable.list > /dev/null"
  )
...
AptInstall(
  ros-${ROS_DISTRO}-controller-manager ros-${ROS_DISTRO}-control-msgs
  ros-${ROS_DISTRO}-control-toolbox ros-${ROS_DISTRO}-moveit-ros-planning-interface
)
...
if(NOT CONAN)
  ...
  execute_process(
    COMMAND /usr/bin/python3 -m pip install --break-system-packages conan>=1.60.1
  ...
  execute_process(
    COMMAND ${CMAKE_COMMAND} -E env CC=${CONAN_C_COMPILER} CXX=${CONAN_CXX_COMPILER}
            ${CONAN} profile new default --detect COMMAND_ERROR_IS_FATAL ANY
  )
  execute_process(
...
if(CONAN_VERSION_OUTPUT MATCHES "Conan version (1\\.[0-9]+\\.[0-9]+)")
  ...
  # Conan 2.x
  file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/conan_global.conf"
...
if(ROS_IS_ROS2)
  ...
  # Dependencies for bota
  AptInstall(
    libxmlrpcpp-dev librosconsole-dev ros-${ROS_DISTRO}-diagnostic-updater
    ros-${ROS_DISTRO}-soem ros-${ROS_DISTRO}-ethercat-grant
  )
